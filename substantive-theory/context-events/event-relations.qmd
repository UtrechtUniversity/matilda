...
When we do not explicitly account for situational influences on our constructs, we implicitly treat those influences as random error. This random error is in turn assumed to cancel itself out - some influences are positive, some negative - over a sufficiently large number of independent observations. 
Depending on the specific research context, this may be a more or less appropriate assumption to make. 
...
- Available resources for the measurement of events (e.g. restrictions of N of prompts per day)
- Participant burden
-->
